<?php
	...
	$post_layout = petsworld_option('pageoptions','post-archives-post-layout');
	...
	$excerpt = petsworld_option('pageoptions','post-archives-excerpt');
	...
	$show_post_format = petsworld_option('pageoptions','post-format-meta'); 
	$show_post_format = isset( $show_post_format )? "" : "hidden";
	
	$show_author_meta = petsworld_option('pageoptions','post-author-meta');
	$show_author_meta = isset( $show_author_meta ) ? "" : "hidden";
	
	$show_date_meta = petsworld_option('pageoptions','post-date-meta');
	$show_date_meta = isset( $show_date_meta ) ? "" : "hidden";	

	$show_comment_meta = petsworld_option('pageoptions','post-comment-meta');
	$show_comment_meta = isset( $show_comment_meta ) ? "" : "hidden";

	$show_category_meta = petsworld_option('pageoptions','post-category-meta');
	$show_category_meta = isset( $show_category_meta ) ? "" : "hidden";
	
	$show_tag_meta = petsworld_option('pageoptions','post-tag-meta');
	$show_tag_meta = isset( $show_tag_meta ) ? "" : "hidden";

	$post_style = petsworld_option('pageoptions','post-style');
	...
	if( have_posts() ) :
                        ...
                        <?php if( $format == "image" || empty($format) ) :
                                if( has_post_thumbnail() ) :?>
                                    <div class="entry-thumb">
                                        <a href="<?php the_permalink(); ?>" title="<?php printf(esc_attr__('Permalink to %s','petsworld'),the_title_attribute('echo=0')); ?>"><?php the_post_thumbnail("full"); ?></a>
                                ...
                                <div class="entry-details">
                                    ...
                                    <div class="entry-date">
                                        ...
                                        if( post_password_required() ) {
                                            echo '<i class="pe-icon pe-lock"> </i>'.esc_html__('Enter your password to view comments.', 'petsworld');
                                        } else {
											comments_popup_link( 
												wp_kses( '<i class="pe-icon pe-chat"> </i>'.esc_html__(' 0','petsworld') , array( 'i' => array('class' => array(), 'id' => array()) ) ),
												wp_kses( '<i class="pe-icon pe-chat"> </i>'.esc_html__(' 1','petsworld') , array( 'i' => array('class' => array(), 'id' => array()) ) ),
                                                wp_kses( '<i class="pe-icon pe-chat"> </i>'.esc_html__(' %','petsworld') , array( 'i' => array('class' => array(), 'id' => array()) ) ), 
                                                '',
                                                wp_kses( '<i class="pe-icon pe-chat"> </i>'.esc_html__(' off','petsworld') , array( 'i' => array('class' => array(), 'id' => array())) ) );
                                        }?>
                                        </div><!-- comment -->                                         
                                     </div><!-- .entry-date -->
                                	...
                                	<div class="entry-title">
                                    	<h4><a href="<?php the_permalink(); ?>" title="<?php printf( esc_attr__('Permalink to %s','petsworld'), the_title_attribute('echo=0')); ?>"><?php the_title(); ?></a></h4>
                                    </div>
                                    
                                   <?php if( isset($allow_excerpt) && isset($excerpt) ):?>
                                    		<div class="entry-body"><?php echo petsworld_excerpt($excerpt); ?></div>
                                   <?php endif;?>
                                   ...
                                   <div class="entry-meta-data">
                                   		...
                                   		<p class="author <?php echo esc_attr( $show_author_meta ); ?>">
                                            ...
                                            <a href="<?php echo get_author_posts_url(get_the_author_meta('ID')); ?>" 
                                            	title="<?php esc_attr_e('View all posts by ', 'petsworld'); echo get_the_author(); ?>"><?php echo get_the_author(); ?></a>
                                        </p>
                                    ...
                                    <?php if( isset($allow_read_more) && isset($read_more) ):
											...
											if( has_shortcode( $sc , 'dt_sc_button' ) ) :
												$sc = do_shortcode($sc);
												echo !empty( $sc ) ? $sc : '';
											else :
												$link = get_permalink( get_the_id() );
												echo '<a href="'.esc_attr($link).'" class="dt-sc-button small icon-right with-icon filled type1">'.esc_html__('Read More','petsworld').'<span class="fa fa-long-arrow-right"> </span></a>';
											endif;
                        ...
                        <?php elseif( $post_style == "entry-date-author-left"):?>
                        		...
                        		<div class="entry-date-author">
                                    ...
                                    <div class="entry-author <?php echo esc_attr( $show_author_meta ); ?>">
                                    	...
                                    	<a href="<?php echo get_author_posts_url(get_the_author_meta('ID')); ?>" 
                                        	title="<?php esc_attr_e('View all posts by ', 'petsworld'); echo get_the_author(); ?>"><span><?php echo get_the_author(); ?></span></a>
                                    </div>
                        ...
                        <?php else: # Default Post Style ?>
                        		<div class="entry-details">
                                    ...
                                    <div class="entry-meta">
                                    	...
                                    	<div class="date <?php echo esc_attr($show_date_meta); ?>"><?php esc_html_e('Posted on','petsworld'); echo get_the_date(' d M Y'); ?></div>
		...
		echo '</div>';?>
        	...
        	<!-- **Pagination** -->
            <div class="pagination blog-pagination"><?php echo petsworld_pagination(); ?></div><!-- **Pagination** -->
            <!-- Blog Template Ends --><?php
		
	else:?>
    	<h2><?php esc_html_e('Nothing Found.', 'petsworld'); ?></h2>
