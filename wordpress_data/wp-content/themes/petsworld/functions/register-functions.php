 ...
 * Theme support
 * --------------------------------------------------------------------------- */
if (!function_exists('petsworld_features')) {

	function petsworld_features() {
	...
	add_action('after_setup_theme', 'petsworld_features');
}

/* ---------------------------------------------------------------------------
 * When theme activation, update default option
 * --------------------------------------------------------------------------- */
if (!function_exists('petsworld_activation_function')) {

	// check activate theme
	function petsworld_activation_function($oldname, $oldtheme=false) {
		if(!in_array($oldname, array('Pets Shop', 'Pets Shop Child'))) {
			update_option(PETSWORLD_SETTINGS, petsworld_default_option());
		}
	}
	add_action("after_switch_theme", "petsworld_activation_function", 10 , 2);
}

/* ---------------------------------------------------------------------------
 *	Under Construction
 * --------------------------------------------------------------------------- */
if( ! function_exists('petsworld_under_construction') ){
	function petsworld_under_construction(){
...
if( petsworld_option('pageoptions','enable-comingsoon') ):
	add_action('template_redirect', 'petsworld_under_construction', 30);

	// getting shortcode css ----------------------
	add_action('wp_enqueue_scripts', 'petsworld_rand_css', 101 );
	if (!function_exists('petsworld_rand_css')) {
	  function petsworld_rand_css() {
		$id = petsworld_option('pageoptions','comingsoon-pageid');
		if ( $id ) {
			$shortcodes_custom_css = get_post_meta( $id, '_wpb_shortcodes_custom_css', true );
			if ( ! empty( $shortcodes_custom_css ) ) {
				
				$css = petsworld_styles_minify( $shortcodes_custom_css );
				
				wp_register_style('vc_shortcodes-custom-'.$id, '', false, THEME_VERSION, 'all' );
				wp_enqueue_style('vc_shortcodes-custom-'.$id);
				wp_add_inline_style('vc_shortcodes-custom-'.$id, $css );
			}
		}
	  }
    }
 ...
 * --------------------------------------------------------------------------- */
if( !function_exists('petsworld_wp_list_categories') ){
	function petsworld_wp_list_categories( $output ){
	...
	add_filter('wp_list_categories', 'petsworld_wp_list_categories');
}

/* ---------------------------------------------------------------------------
 * Filter to modify default list archive widget view
 * --------------------------------------------------------------------------- */
if( !function_exists('petsworld_wp_list_archive') ){
	function petsworld_wp_list_archive( $link_html,$url, $text, $format, $before, $after ) {
	...
	add_filter('get_archives_link', 'petsworld_wp_list_archive', 10, 6);	
}

/* ---------------------------------------------------------------------------
 * Filter to execute shortcode inside contact form7
 * --------------------------------------------------------------------------- */
if( !function_exists('petsworld_wpcf7_form_elements') ){
	function petsworld_wpcf7_form_elements($form) {
		$form = do_shortcode( $form );
		return $form;
	}
	add_filter('wpcf7_form_elements', 'petsworld_wpcf7_form_elements');
}

/* ---------------------------------------------------------------------------
 * Get Current Page or Page ID
 * --------------------------------------------------------------------------- */
if (!function_exists('petsworld_ID')) {
function petsworld_ID(){
	global $post;
	$postID = false;
 ...
 * --------------------------------------------------------------------------- */
if (!function_exists('petsworld_slider')) {
function petsworld_slider( $id = false ){
	...
	if( $id || is_home() || get_post_type() == 'page' ) {
		...
		elseif( is_page() ) $id = petsworld_ID();
		...
		if( is_array( $slider_key) && array_key_exists('slider_type', $slider_key) ) {
			...
			if( ( $slider_key['slider_type'] == 'revolutionslider' ) && ( array_key_exists( 'revolutionslider_id', $slider_key ) ) && ( array_key_exists( 'show_slider', $slider_key ) ) ) {
				...
				$slider = '<div id="slider"><div class="dt-sc-main-slider" id="dt-sc-custom-slider">';
					$slider .= petsworld_wp_kses(do_shortcode( $slider_key['customslider_sc'] ));
				$slider .= '</div></div>';
			}
		}
	}
 ...
 * --------------------------------------------------------------------------- */
if (!function_exists('petsworld_pagination')) {
function petsworld_pagination( $query = false, $load_more = false ){
	global $wp_query;
	...
	$end_size = 1;
	$mid_size = 2;
	$show_all = petsworld_option('general','showall-pagination');
	$dots = false;
		...
		if( $load_more ){
			...
			if( $paged < $total ){
				$output .= '<div class="column one pager_wrapper pager_lm">';
					$output .= '<a class="pager_load_more button button_js" href="'. get_pagenum_link( $next ) .'">';
						...
						$output .= '<span class="button_label">'. esc_html__('Load more', 'petsworld') .'</span>';
					$output .= '</a>';
				$output .= '</div>';
			}
			...
			$output .= '<div class="column one pager_wrapper">';
				...
				$args = array(
					...
					'mid_size'           => $mid_size,
					'prev_next'          => true,
					'prev_text'          => '<i class="fa fa-angle-double-left"></i>'.esc_html__('Prev', 'petsworld'),
					'next_text'          => esc_html__('Next', 'petsworld').'<i class="fa fa-angle-double-right"></i>',
					'type'               => 'list'
				);
 ...
 * Page Title
 * --------------------------------------------------------------------------- */
if( ! function_exists( 'petsworld_page_title' ) ){
	function petsworld_page_title( $echo = false ){
		...
		if( function_exists('tribe_is_month') && ( tribe_is_event_query() || tribe_is_month() || tribe_is_event() || tribe_is_day() || tribe_is_venue() ) ){
			...
			$title = get_the_title( petsworld_ID() );
 ...
 * Breadcrumbs
 * --------------------------------------------------------------------------- */
if (!function_exists('petsworld_breadcrumbs')) {
function petsworld_breadcrumbs(){
	global $post;
	
	$homeLink = esc_url( home_url('/') );
	$separator = '<span class="'.petsworld_option('layout','breadcrumb-delimiter').'"></span>';
	...
	if( function_exists('is_bbpress') && is_bbpress() ){
		bbp_breadcrumb( array(
			...
			'after' 		=> '</div>',
			'sep' 			=> '<span class="'.petsworld_option('layout','breadcrumb-delimiter').'"></span>',
			'crumb_before' 	=> '',
			'crumb_after' 	=> '',
			'home_text' 	=> esc_html__('Home','petsworld'),
		) );
		return true;
	}
	...
	$breadcrumbs[] =  $separator.'<a href="'. $homeLink .'">'. esc_html__('Home','petsworld') .'</a>';
	...
	if( function_exists('tribe_is_month') && ( tribe_is_event_query() || tribe_is_month() || tribe_is_event() || tribe_is_day() || tribe_is_venue() ) ) {
		...
		$breadcrumbs[] = '<a href="">'.esc_html__('Search', 'petsworld').'</a>';

	} elseif( is_single() && ! is_attachment() ){

		if( get_post_type() == 'dt_portfolios' ){

			$cat = get_the_term_list(petsworld_ID(), 'portfolio_entries', '', '$$$', '');
			...
			$terms = get_the_terms( petsworld_ID(), 'product_cat' );
		...
		$breadcrumbs[] = '<span class="current">'. get_the_title( petsworld_ID() ) .'</span>';
		...
		foreach( $breadcrumbs as $bk => $bc ){
			if( !is_object( $bc ) ) {
				if( strpos( $bc , $separator ) ) {
					...
					echo apply_filters( 'petworld_breadcrumbs', $bc );
				} else {
					if( $i == $count ) $separator = '';
					echo apply_filters( 'petworld_breadcrumbs', $bc . $separator );
				}
			}
			$i++;
		}
 ...
 * Breadcrumb Section
 * --------------------------------------------------------------------------- */
if (!function_exists('petsworld_breadcrumb_section')) {
function petsworld_breadcrumb_section($id, $post_type ) {

	$bcrumb = petsworld_option('layout','show-breadcrumb');
	if( isset($bcrumb) && $bcrumb == 'true' ) :
		...
		if(!empty($bg)) :
			...
			$color = !empty($settings['sub-title-bg-color']) ? petsworld_hex2rgb($settings['sub-title-bg-color']) : '';
		else:
			$bg = petsworld_option('layout','sub-title-bg');
			$opacity = petsworld_option('layout','sub-title-opacity');
			$opacity = !empty($opacity) ? $opacity : '1';
			$position = petsworld_option('layout','sub-title-bg-position');
			$position = !empty($position) ? $position : 'center center';
			$repeat = petsworld_option('layout','sub-title-bg-repeat');
			$repeat = !empty($repeat) ? $repeat : 'repeat';
			$color = petsworld_option('layout','sub-title-bg-color');
			$color = !empty($color) ? petsworld_hex2rgb($color) : '';
		endif;

		$style = !empty($bg) ? "background:url($bg) $position $repeat;" : "";
		$style .= !empty($color) ? "background-color:rgba(  $color[0] ,  $color[1],  $color[2], {$opacity});" : "";

		$bstyle = petsworld_option('layout','breadcrumb-style');
		$bstyle .= petsworld_option('layout','breadcrumb-darkbg') ? ' dt-sc-dark-bg ' : '';
...
if (!function_exists('petsworld_breadcrumb_section_with_class')) {
function petsworld_breadcrumb_section_with_class ( $title , $class ){
	$bcrumb = petsworld_option('layout','show-breadcrumb');
	if( isset($bcrumb) && $bcrumb == 'true' ) :
...
if (!function_exists('petsworld_events_title')) {
function petsworld_events_title() {
	...
	if( tribe_is_month() && !is_tax() ) { 
		$title = sprintf( esc_html__( 'Events for %s', 'petsworld' ), date_i18n( 'F Y', strtotime( tribe_get_month_view_date() ) ) );
	} elseif( class_exists('Tribe__Events__Pro__Main') && tribe_is_week() )  {
		$title = sprintf( esc_html__('Events for week of %s', 'petsworld'), date_i18n( $date_format, strtotime( tribe_get_first_week_day($wp_query->get('start_date') ) ) ) );
	} elseif( class_exists('Tribe__Events__Pro__Main') && tribe_is_day() ) {
		$title = esc_html__( 'Events for', 'petsworld' ) . ' ' . date_i18n( $date_format, strtotime( $wp_query->get('start_date') ) );
	} elseif( class_exists('Tribe__Events__Pro__Main') && (tribe_is_map() || tribe_is_photo()) ) {
		if( tribe_is_past() ) {
			$title = esc_html__( 'Past Events', 'petsworld' );
		} else {
			$title = esc_html__( 'Upcoming Events', 'petsworld' );
		}
		...
		$title = esc_html__('Tag Archives','petsworld');
	}
	return $title;
}
}
 ...
 * Excerpt
 * --------------------------------------------------------------------------- */
if (!function_exists('petsworld_excerpt')) {
function petsworld_excerpt($limit = NULL) {
 ...
 * --------------------------------------------------------------------------- */
if (!function_exists('petsworld_wp_kses')) {
function petsworld_wp_kses($content) {
 ...
 * --------------------------------------------------------------------------- */
if(!function_exists('petsworld_hex2rgb')) {

	function petsworld_hex2rgb($hex) {
		$hex = str_replace ( "#", "", $hex );
 ...
 * --------------------------------------------------------------------------- */
if( ! function_exists( 'petsworld_opts_get' ) ) {

	function petsworld_opts_get( $opt_name, $default = null ){

		// getting option
		$opts = get_option ( PETSWORLD_SETTINGS );
		$opts = is_array( $opts ) ? $opts : array();

		$value = petsworld_array_search($opts, $opt_name);
		if( ! empty( $value ) ){
			return $value;
		} else {
			return $default;
 ...
 * --------------------------------------------------------------------------- */
if( ! function_exists( 'petsworld_opts_show' ) ) {

	function petsworld_opts_show( $opt_name, $default = null ){
 ...
 * --------------------------------------------------------------------------- */
if (!function_exists('petsworld_array_search')) {
function petsworld_array_search(array $array, $search) {
	$keys = false;
	...
	foreach ($array as $key => $value) {
		if (is_array($array[$key])) {
			if (array_key_exists($search, $array[$key])) {
			    ...
			    $keys = petsworld_array_search($array[$key], $search);
			}
		}
	}
	return $keys;
 ...
 * Theme Header Logo
 * --------------------------------------------------------------------------- */
if (!function_exists('petsworld_header_logo')) {
function petsworld_header_logo() {
	echo '<div id="logo">';
		if( petsworld_option('layout', 'logo') ):
			$url = petsworld_option('layout', 'logo-url');
			$url = !empty( $url ) ? $url : PETSWORLD_THEME_URI . "/images/logo.png";

			$retina_url = petsworld_option('layout','retina-logo-url');
			$retina_url = !empty($retina_url) ? $retina_url : PETSWORLD_THEME_URI ."/images/logo@2x.png";

			$width = petsworld_option('layout','retina-logo-width');
			$width = !empty($width) ? $width."px;" : "140px";

			$height = petsworld_option('layout','retina-logo-height');
 ...
 * Theme Comment Style
 * --------------------------------------------------------------------------- */
if (!function_exists('petsworld_comment_style')) {
function petsworld_comment_style( $comment, $args, $depth ) {
	...
	case 'trackback':
		...
		echo "<p>";
		_e('Pingback:', 'petsworld');
		comment_author_link();
		edit_comment_link( esc_html__('Edit', 'petsworld'), ' ', '');
		echo "</p>";
		break;
	...
	case '':
		...
		echo '  <div class="comment-body">';
					...
					if ($comment->comment_approved == '0') :
						_e('Your comment is awaiting moderation.', 'petsworld');
					endif;
					edit_comment_link(esc_html__('Edit', 'petsworld'));
		echo '	</div>';
		echo '	<div class="reply">';
		echo 		comment_reply_link(array_merge($args, array('reply_text' => esc_html__('Reply', 'petsworld'), 'depth' => $depth, 'max_depth' => $args['max_depth'])));
		echo '	</div>';
		echo '</section>';
 ...
 * Theme show sidebar
 * --------------------------------------------------------------------------- */
if (!function_exists('petsworld_show_sidebar')) {
function petsworld_show_sidebar( $type, $id, $position = 'right' ) {
 ...
 * Theme Check sidebar
 * --------------------------------------------------------------------------- */
if (!function_exists('petsworld_check_sidebar')) {
function petsworld_check_sidebar( $type, $id, $position = 'right' ) {
 ...
 * --------------------------------------------------------------------------- */
if (!function_exists('petsworld_global_variables')) {
function petsworld_global_variables($variable = '') {

	global $woocommerce, $product, $woocommerce_loop, $post, $wp_query, $pagenow, $petsworld_like_love;
	...
	switch($variable) {
		...
		case 'pagenow':
			return $pagenow;
		break;
		case 'petsworld_like_love':
			return $petsworld_like_love;
		break;
	}
	return false;
}
