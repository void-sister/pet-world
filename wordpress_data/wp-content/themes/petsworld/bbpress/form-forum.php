			...
			<fieldset class="bbp-form">
				<legend>
					...
					<?php
						if ( bbp_is_forum_edit() )
							printf( esc_html__( 'Now Editing &ldquo;%s&rdquo;', 'petsworld' ), bbp_get_forum_title() );
						else
							bbp_is_single_forum() ? printf( esc_html__( 'Create New Forum in &ldquo;%s&rdquo;', 'petsworld' ), bbp_get_forum_title() ) : esc_html_e('Create New Forum', 'petsworld' );
					?>
				...
				<?php if ( !bbp_is_forum_edit() && bbp_is_forum_closed() ) : ?>
					...
					<div class="bbp-template-notice">
						<p><?php esc_html_e('This forum is closed to new content, however your account still allows you to do so.', 'petsworld' ); ?></p>
					</div>
				...
				<div>
					...
					<p>
						<label for="bbp_forum_title"><?php printf( esc_html__( 'Forum Name (Maximum Length: %d):', 'petsworld' ), bbp_get_title_max_length() ); ?></label><br />
					...
					<p>
						<label for="bbp_forum_type"><?php esc_html_e('Forum Type:', 'petsworld' ); ?></label><br />
					...
					<p>
						<label for="bbp_forum_status"><?php esc_html_e('Status:', 'petsworld' ); ?></label><br />
					...
					<p>
						<label for="bbp_forum_visibility"><?php esc_html_e('Visibility:', 'petsworld' ); ?></label><br />
					...
					<p>
						<label for="bbp_forum_parent_id"><?php esc_html_e('Parent Forum:', 'petsworld' ); ?></label><br />

						<?php
							bbp_dropdown( array(
								'select_id' => 'bbp_forum_parent_id',
								'show_none' => esc_html__( '(No Parent)', 'petsworld' ),
					...
					<div class="bbp-submit-wrapper">
						...
						<button type="submit" tabindex="<?php bbp_tab_index(); ?>" id="bbp_forum_submit" name="bbp_forum_submit" class="button submit"><?php esc_html_e('Submit', 'petsworld' ); ?></button>
...
<?php elseif ( bbp_is_forum_closed() ) : ?>
	...
	<div id="no-forum-<?php bbp_forum_id(); ?>" class="bbp-no-forum">
		<div class="bbp-template-notice">
			<p><?php printf( esc_html__( 'The forum &#8216;%s&#8217; is closed to new content.', 'petsworld' ), bbp_get_forum_title() ); ?></p>
		</div>
	</div>
...
<?php else : ?>
	...
	<div id="no-forum-<?php bbp_forum_id(); ?>" class="bbp-no-forum">
		<div class="bbp-template-notice">
			<p><?php is_user_logged_in() ? esc_html_e('You cannot create new forums.', 'petsworld' ) : esc_html_e('You must be logged in to create new forums.', 'petsworld' ); ?></p>
		</div>
	</div>
