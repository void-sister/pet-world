...
$gradient_color_1 = $gradient_color_2 = $gradient_custom_color_1 = $gradient_custom_color_2 = $gradient_text_color = $image = $hover_image = '';
...
if ( 'true' === $add_icon ) {
	...
	if( $i_type !== "image" ) {
		...
		if( !empty( $image ) ) {
			$img = wpb_getImageBySize( array( 'attach_id' => $image , 'thumb_size' => 'full', 'class' => 'default-image') );
			$img = $img['thumbnail'];
		}
		
		if( !empty( $hover_image ) ) {
			$h_img = wpb_getImageBySize( array( 'attach_id' => $hover_image , 'thumb_size' => 'full', 'class' => 'hover-image') );
			$h_img = $h_img['thumbnail'];
		}
		
		$icon_html =  '<span>'.$img.$h_img.'</span>';
		$button_classes[] = 'with-image';		
	}
