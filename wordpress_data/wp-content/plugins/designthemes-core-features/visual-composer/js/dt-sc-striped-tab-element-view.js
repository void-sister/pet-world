(function ($) {
	...
	window.DTStripedTabView = window.VcColumnView.extend( {
		events: {
		...
		render: function () {
			...
			window.DTStripedTabView.__super__.render.call( this );
			/**
			 * @deprecated 4.4.3
		...
		ready: function ( e ) {
			window.DTStripedTabView.__super__.ready.call( this, e );
		...
		changeShortcodeParams: function ( model ) {
			...
			window.DTStripedTabView.__super__.changeShortcodeParams.call( this, model );
		...
		cloneModel: function ( model, parent_id, save_order ) {
			...
			if ( 'dt_sc_petshop_striped_tab_element' === tag ) {
				_.extend( params,
					{ tab_id: Date.now() + '-' + this.$tabs.find( '[data-element_type=dt_sc_petshop_striped_tab_element]' ).length + '-' + Math.floor( Math.random() * 11 ) } );
			}
