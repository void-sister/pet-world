(function ($) {
	...
	window.DTStripedTabsView = vc.shortcode_view.extend( {
		...
		initialize: function ( params ) {
			window.DTStripedTabsView.__super__.initialize.call( this, params );
			_.bindAll( this, 'stopSorting' );
		},
		render: function () {
			window.DTStripedTabsView.__super__.render.call( this );
			this.$tabs = this.$el.find( '.wpb_tabs_holder' );
			this.createAddTabButton();
			return this;
		},
		ready: function ( e ) {
			window.DTStripedTabsView.__super__.ready.call( this, e );
		},
		...
		addTab: function ( e ) {
			...
			var tab_title = window.i18nLocale.tab,
				tabs_count = this.$tabs.find( '[data-element_type=dt_sc_petshop_striped_tab_element]' ).length,
				tab_id = (Date.now() + '-' + tabs_count + '-' + Math.floor( Math.random() * 11 ));
			vc.shortcodes.create( {
				shortcode: 'dt_sc_petshop_striped_tab_element',
		...
		cloneModel: function ( model, parent_id, save_order ) {
			...
			if ( 'dt_sc_petshop_striped_tab_element' === tag ) {
				_.extend( params,
					{ tab_id: Date.now() + '-' + this.$tabs.find( '[data-element-type=dt_sc_petshop_striped_tab_element]' ).length + '-' + Math.floor( Math.random() * 11 ) } );
			}
			...
			model_clone = Shortcodes.create( {
				...
				parent_id: parent_id,
				order: new_order,
				cloned: ('dt_sc_petshop_striped_tab_element' !== tag), // TODO: review this by @say2me
