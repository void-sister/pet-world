<?php
class PetsWorld_Twitter extends WP_Widget {
	#1.constructor
	function __construct() {
		$widget_options = array("classname"=>'widget_tweetbox', 'description'=>__('To show latest twitter tweets', 'petsworld'));
		parent::__construct(false,THEME_NAME.' '.__('Twitter Widget','petsworld'),$widget_options);
	}

	#2.widget input form in back-end
	function form($instance) {
		$instance = wp_parse_args( (array) $instance,array( 'title' => __('Latest Tweets','petsworld'), 'count' => '3', 'username' => '',
        ...
        <p><label for="<?php echo ($this->get_field_id('consumer_key')); ?>"><?php _e('Consumer Key:','petsworld');?> 
        ...
        <p><label for="<?php echo ($this->get_field_id('consumer_secret')); ?>"><?php _e('Consumer Secret:','petsworld');?> 
        ...
        <p><label for="<?php echo ($this->get_field_id('access_token')); ?>"><?php _e('Access Token:','petsworld');?> 
        ...
        <p><label for="<?php echo ($this->get_field_id('access_token_secret')); ?>"><?php _e('Access Token Secret:','petsworld');?> 
        ...
        <p><label for="<?php echo ($this->get_field_id('username')); ?>"><?php _e('Enter your twitter username:','petsworld');?>
        ...
        <p><label for="<?php echo ($this->get_field_id('count')); ?>"><?php _e('How many entries do you want to show:','petsworld');?>
        ...
        <p><input type="checkbox"  id="<?php echo ($this->get_field_id('exclude_replies'));?>" name="<?php echo ($this->get_field_name('exclude_replies'));?>" 
			<?php checked($exclude_replies); ?> /> <label for="<?php echo ($this->get_field_id('exclude_replies')); ?>"><?php _e('Exclude @replies','petsworld');?></label></p>
            
        <p><input type="checkbox"  id="<?php echo ($this->get_field_id('time'));?>" name="<?php echo ($this->get_field_name('time'));?>" 
			<?php checked($time); ?> /> <label for="<?php echo ($this->get_field_id('time')); ?>"><?php _e('Show time of tweet','petsworld');?></label></p>
            
        <p><input type="checkbox"  id="<?php echo ($this->get_field_id('display_avatar'));?>" name="<?php echo ($this->get_field_name('display_avatar'));?>" 
				<?php checked($display_avatar); ?> /> <label for="<?php echo ($this->get_field_id('display_avatar')); ?>"><?php _e('Show user avatar','petsworld');?></label></p>		
<?php
	}
		...
		echo ($before_widget);			
			...
			if($username && $consumer_key && $consumer_secret && $access_token && $access_token_secret && $count) { 
				...
				if($twitter && is_array($twitter)) {
					...
					echo '<li>'.__('No public Tweets found','petsworld').'</li>';
				}
				echo "</ul>";
			}
		echo ($after_widget);
