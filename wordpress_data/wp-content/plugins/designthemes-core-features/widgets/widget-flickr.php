<?php 
class PetsWorld_Flickr extends WP_Widget {
	#1.constructor
	function __construct() {
		$widget_options = array("classname"=>'widget_flickr', 'description'=>__('A widget that show last flickr photo streams', 'petsworld'));
		parent::__construct(false,THEME_NAME.' '.__(' Flickr Widget','petsworld'),$widget_options);
	}
	...
	function form($instance) {
        ...
        <p><label for="<?php echo ($this->get_field_id('title')); ?>"><?php esc_html_e('Title:','petsworld');?> 
		   <input class="widefat" id="<?php echo ($this->get_field_id('title')); ?>" name="<?php echo ($this->get_field_name('title')); ?>" 
            type="text" value="<?php echo esc_attr($title); ?>" /></label></p>
           
        <p><label for="<?php echo ($this->get_field_id('flickr_id')); ?>"><?php esc_html_e('Flickr ID:','petsworld');?>
        ...
        <p><label for="<?php echo ($this->get_field_id('count')); ?>"><?php esc_html_e('How many entries do you want to show:','petsworld');?>
         ...
         <p><label for="<?php echo ($this->get_field_id('show'));?>"><?php esc_html_e('What pictures to display','petsworld'); ?>
         	<select class="widefat" id="<?php echo ($this->get_field_id('show')); ?>" name="<?php echo ($this->get_field_name('show')); ?>">
            <?php  $a = array("latest"=> __("Latest",'petsworld'),"random"=>__("Random",'petsworld'));
         ...
         <p><label for="<?php echo ($this->get_field_id('size'));?>"><?php esc_html_e('What pictures to display','petsworld'); ?>
         	<select class="widefat" id="<?php echo ($this->get_field_id('size')); ?>" name="<?php echo ($this->get_field_name('size')); ?>">
            <?php  $a = array("s"=> __("square",'petsworld'),"t"=> __("thumbnail",'petsworld'), "m" => __("medium",'petsworld'));
			foreach($a as $key => $value ):
