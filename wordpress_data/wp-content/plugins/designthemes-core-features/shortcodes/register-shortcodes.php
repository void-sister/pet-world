...
if (! class_exists ( 'DTCoreShortcodes' )) {
	...
	class DTCoreShortcodes {
		...
		function dt_wp_enqueue_scripts() {
			...
			wp_enqueue_script ( 'dt-sc-scrollto', plugin_dir_url ( __FILE__ ) . 'js/jquery.scrollto.js', array (), false, true );
		...
		function dt_sc_mc_subscribe() {
			if( defined('PETSWORLD_CORE_PLUGIN') ) {
				require_once( PETSWORLD_CORE_PLUGIN."/apis/mailchimp/mailchimp.php" );
			}
		}
